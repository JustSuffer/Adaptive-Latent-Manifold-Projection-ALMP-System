    ...
    >
      ...
      <div className="flex flex-col gap-3">
        ...
        <label className="flex flex-col gap-1 cursor-pointer group">
          <span className="text-[10px] text-slate-500 uppercase tracking-widest ml-1">Model Weights (.pt)</span>
          <div className={`flex items-center gap-3 p-3 rounded-xl border transition-all duration-300 ${modelFile ? 'bg-cyber-neon/10 border-cyber-neon/30 text-cyber-neon' : 'bg-white/5 border-white/10 text-slate-300 group-hover:bg-white/10 group-hover:border-white/20'}`}>
            <Cpu size={18} />
            <span className="text-xs font-mono truncate flex-1">
              {modelFile ? modelFile.name : "Select Model File..."}
            </span>
          </div>
          <input type="file" accept=".pt,.safetensors" className="hidden" onChange={handleModelChange} />
        </label>

        {/* Image Input */}
        <label className="flex flex-col gap-1 cursor-pointer group">
          <span className="text-[10px] text-slate-500 uppercase tracking-widest ml-1">Inference Target (.jpg/.png)</span>
          <div className={`flex items-center gap-3 p-3 rounded-xl border transition-all duration-300 ${imageFile ? 'bg-cyber-blue/10 border-cyber-blue/30 text-cyber-blue' : 'bg-white/5 border-white/10 text-slate-300 group-hover:bg-white/10 group-hover:border-white/20'}`}>
            <ImageIcon size={18} />
            <span className="text-xs font-mono truncate flex-1">
      ...
      <button 
        ...
        className={`mt-2 flex items-center justify-center gap-2 p-3 rounded-xl border text-xs font-mono uppercase tracking-widest transition-all duration-300 ${
          isUploading ? 'bg-slate-800/50 border-white/5 text-slate-500 cursor-not-allowed' :
          (!modelFile || !imageFile) ? 'bg-white/5 border-white/10 text-slate-500 cursor-not-allowed' :
